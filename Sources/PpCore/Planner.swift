...
public struct PlanStep: Codable, Equatable, Sendable {
    ...
    /// Short description shown in the widget and given to pp as the step.
...
/// Turns one spoken utterance into ordered steps.
/// Defaults to 100% offline GrammarPlanner; optionally queries a user-specified custom API if enabled in Settings.
public enum Planner {
    public static let defaultModel = "custom-planner"
    /// Overridable: `defaults write local.pp PlannerModel <model id>`.
    ...
    public static var endpoint: URL {
        if let raw = UserDefaults.standard.string(forKey: "PlannerEndpoint"), let url = URL(string: raw) {
            return url
        }
        return URL(string: "http://127.0.0.1:8000/v1/chat/completions")!
    }

    /// Primary plan entrypoint: uses fast local GrammarPlanner by default; falls back to custom API if configured.
    public static func plan(
        utterance: String,
        frontApp: String,
        runningApps: [String],
        apiKey: String? = nil
    ) async throws -> (steps: [PlanStep], usage: Usage?, model: String) {
        // If a custom API key is present and enabled, query user's configured endpoint
        if let key = apiKey, !key.isEmpty, isCustomPlannerEnabled {
            return try await planWithAPI(utterance: utterance, frontApp: frontApp, runningApps: runningApps, apiKey: key)
        }

        // Default: 100% offline local GrammarPlanner
        let localSteps = GrammarPlanner.plan(utterance: utterance, frontApp: frontApp, runningApps: runningApps)
        guard !localSteps.isEmpty else { throw PlannerError.invalidResponse }
        return (steps: localSteps, usage: Usage(prompt: 0, completion: 0, reasoning: 0), model: "local-grammar")
    }

    /// Queries user's custom API endpoint (OpenAI, OpenRouter, Ollama, etc.)
    public static func planWithAPI(
        utterance: String,
        frontApp: String,
        runningApps: [String],
        apiKey: String
        ...
        request.setValue("pp", forHTTPHeaderField: "X-Title")
...
public enum PlannerError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .service(let status, let message):
            switch status {
            case 401: return "Planner service rejected the API key. Check it in Settings."
            case 402: return "Planner service reports no credit for this key."
            case 429: return "Planner service rate limit was reached. Try again shortly."
            default: return "Planner service returned HTTP \(status). \(message ?? "Nothing was executed.")"
            }
        }
    }
}
