...
@main
@MainActor
struct PpDesktopApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var delegate
    @StateObject private var model = AppModel.shared

    var body: some Scene {
        MenuBarExtra("pp", systemImage: model.isBusy ? "waveform" : "waveform.circle") {
            Text(model.headline)
            ...
            Button("Quit pp") { NSApplication.shared.terminate(nil) }.keyboardShortcut("q")
        }
    }
}
...
final class AppModel: ObservableObject {
    ...
    private let log = Logger(subsystem: "local.pp", category: "status")
    ...
    func initializeLocalModel() {
        if let verifiedDir = ModelManager.findVerifiedModel() {
            do {
                let provider = try LayaDecisionProvider.load(from: verifiedDir)
                PpClient.provider = provider
                provider.warmUp()
                modelStatusMessage = nil
                log.notice("Loaded verified local LayaDecisionProvider from \(verifiedDir.path, privacy: .public)")
                return
            } catch {
                modelStatusMessage = "Failed to load Laya model: \(error.localizedDescription)"
                log.error("Failed to load LayaDecisionProvider from \(verifiedDir.path, privacy: .public): \(error.localizedDescription, privacy: .public)")
            }
        }

        // Secondary fallback to locate local directory if checksum cache needs refresh
        let localPaths = [
            FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?.appendingPathComponent("pp/models/laya-mlx"),
        ...
        for path in localPaths {
            if FileManager.default.fileExists(atPath: path.appendingPathComponent("model.safetensors").path) {
                do {
                    let provider = try LayaDecisionProvider.load(from: path)
                    PpClient.provider = provider
                    provider.warmUp()
                    modelStatusMessage = nil
                    log.notice("Loaded local LayaDecisionProvider from \(path.path, privacy: .public)")
                    return
                } catch {
                    log.error("Failed to load LayaDecisionProvider from \(path.path, privacy: .public): \(error.localizedDescription, privacy: .public)")
                }
            }
        }

        modelStatusMessage = "Local Laya 421M model not loaded. Download weights in Settings or run build.sh."
        log.warning("No local Laya model found.")
    }
    ...
    func start() {
        ...
        commandObserver = DistributedNotificationCenter.default().addObserver(
            forName: Notification.Name("local.pp.command"), object: nil, queue: .main
        ) { [weak self] notification in
        ...
        speech.onFinal = { [weak self] text in
            ...
            if self.waitingForWake {
                guard let command = WakePhrase.command(in: text, after: "Hey pp") else {
                    self.beginSpeech()
                    return
                }
                self.waitingForWake = false
                if command.isEmpty {
                    self.beginSpeech()
                    return
                }
        ...
        keyTask = Task {
            do {
                ...
                let planner = try? await Task.detached(priority: .userInitiated) { try KeyStore.read(KeyStore.customPlanner) }.value
                ...
                isLoadingKey = false
                if shortcutReady {
                    headline = isModelReady ? "Finish setup" : "Model or API key needed"
    ...
    func savePlannerKey(_ value: String) {
        do {
            ...
            plannerKey = try KeyStore.read(KeyStore.customPlanner)
            hasPlannerKey = plannerKey != nil
            UserDefaults.standard.set(true, forKey: "CustomPlannerEnabled")
            headline = "Custom planner key saved"
            detail = "pp will use your custom API endpoint to plan complex commands."
        } catch { fail(error.localizedDescription) }
    }
    ...
    private func prepare() -> NSRunningApplication? {
        refreshPermissions()
        guard !isLoadingKey else { fail("Approve pp's saved-key prompt in Keychain, then try again."); return nil }
        guard isModelReady else { fail(modelStatusMessage ?? "Configure your decision service in Settings."); showSettings(); return nil }
        guard accessibilityAllowed else {
            fail("macOS has not recognised this app's Accessibility grant. If pp is already enabled, remove its old entry and add the current app again.")
            showSettings()
            return nil
        }
    ...
    func beginSpeech(holdingShortcut: Bool = false) {
        ...
        let listening = Task {
            do { try await speech.start(handsFree: continuous && !holdingShortcut, wakePhrase: waiting ? "Hey pp" : nil) }
    ...
    private func executePlanned(_ step: PlanStep, goal: String, app first: NSRunningApplication, cached: DesktopSnapshot?, label: String,
        ...
        while true {
            ...
            let stepSafety = SafetyCritic.evaluate(step: step, goal: goal)
            if case .requiresConfirmation(_, let reason) = stepSafety, probability < 0.90 {
                headline = "Confirm: \(step.summary)?"
                detail = "\(reason). Repeat command or speak again to proceed."
                priorCommand = goal
                awaitingClarification = true
                clearPixels(); isBusy = false
                return .stopped
            }
            if case .vetoed(let reason) = stepSafety {
                headline = "Blocked: \(step.summary)"
                detail = reason
                clearPixels(); isBusy = false
                return .stopped
            }
    ...
    private func runCycles(_ goal: String, app first: NSRunningApplication, generation current: UUID, modelSeconds: inout Double) async throws -> StepOutcome {
        ...
        for cycle in 1...14 {
            ...
            // One list feeds the element table and the click head; with the inputs it stays under the 255-option limit.
            ...
            // Evaluate SafetyCritic across destructive, outward-effect, and financial actions
            let safetyVerdict = SafetyCritic.evaluate(
                actionLabel: targetCandidate?.label ?? op.id,
                actionDetail: targetCandidate?.detail ?? "",
                command: goal,
                confidence: target?.confidence ?? op.confidence
            )
            if case .requiresConfirmation(_, let reason) = safetyVerdict {
                headline = "Confirm: \(targetCandidate?.label ?? op.id)?"
                detail = "\(reason). Repeat command or speak again to proceed."
                priorCommand = goal
                awaitingClarification = true
                clearPixels(); isBusy = false
            ...
            if case .vetoed(let reason) = safetyVerdict {
                headline = "Blocked"
                detail = reason
                clearPixels(); isBusy = false
                return .stopped
            }
            if let target, target.confidence < 0.2 {
    ...
    func showVoiceWidget() {
        ...
        if !isBusy {
            ...
            detail = setupComplete ? "Hold \(shortcut.label) to speak. Release to act." : "Open Settings from the pp menu bar icon."
            transcript = ""
            timing = ""
    ...
    func toggleHandsFree() {
        ...
        cancel(showStatus: false)
        handsFree = true
        waitingForWake = wakeWordEnabled
        beginSpeech()
    }
...
private struct SettingsView: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 22) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text("pp").font(.title2.bold())
                    Text("Say it. Your Mac acts.").foregroundStyle(.secondary)
                }
            }
            VStack(alignment: .leading, spacing: 8) {
                Text("1. Local Decision Engine").font(.headline)
                HStack {
                    Image(systemName: model.isLocalModelLoaded ? "checkmark.circle.fill" : "exclamationmark.triangle.fill")
                        .foregroundStyle(model.isLocalModelLoaded ? .green : .orange)
                    Text(model.isLocalModelLoaded ? "Laya 421M Ready (Apple Silicon Metal)" : (model.modelStatusMessage ?? "Model Not Loaded"))
                        .font(.subheadline)
                    Spacer()
                    if !model.isLocalModelLoaded {
                ...
                Text("Custom API (Optional BYO-API)").font(.subheadline.bold())
                HStack {
                    SecureField(model.hasPlannerKey ? "Planner key saved — enter a replacement" : "Custom Planner API key (optional)", text: $plannerKey)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Try “Open Desktop”, “Open Brave, go to google.com and type in hello”, or “Open Codex and type this: hello”.")
                    .font(.caption).foregroundStyle(.secondary)
                Toggle("Invoke the widget by saying “Hey pp”", isOn: $model.wakeWordEnabled)
                Text("When enabled, listens in the background while the app is running, including after launch. Say “Hey pp” to open the widget and start hands-free. Closing the widget returns to wake-word listening; Stop or Escape pauses all listening. Apple Speech may process audio online.")
                    .font(.caption).foregroundStyle(.secondary)
        ...
        .frame(width: 540, height: min(760, (NSScreen.main?.visibleFrame.height ?? 840) - 80))
        .onReceive(NotificationCenter.default.publisher(for: NSWindow.willCloseNotification)) { notification in
            if let window = notification.object as? NSWindow, window.title == "pp", model.recordingShortcut { model.finishShortcutRecording(nil) }
        }
