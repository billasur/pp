...
enum Desktop {
    ...
    private static let log = Logger(subsystem: "local.pp", category: "desktop")
    ...
    static func capture(application: NSRunningApplication, command: String, dictation: String? = nil, includeMenus: Bool = true) async throws -> DesktopSnapshot {
        guard hasAccess else { throw DesktopError(message: "Enable Accessibility for pp in System Settings.") }
        ...
        if !typingOnly {
            ...
            if let desktop = manager.urls(for: .desktopDirectory, in: .userDomainMask).first {
                do {
                    ...
                    folderAccessError = "Desktop folders could not be read. Allow Desktop folder access for pp in System Settings → Privacy & Security → Files and Folders."
                }
            }
        }
